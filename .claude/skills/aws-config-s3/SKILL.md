...
description: >
  ...
  inside `infra/aws/tf/config/aws/config.json` for the CSV Data pipeline. Trigger 
  when the user wants to: set the S3 bucket name,
...
This skill governs the `aws.s3` block in `infra/aws/tf/config/aws/config.json`.
